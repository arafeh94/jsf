...
public class EigenvectorCentralityAnalysis extends DataAnalysisBase {

    @Override
    public Table analyse() {
        Table table = new Table("Page Rank Results");
        try {
            for (Pair<String, Double> pair : graphNodeBll.eigenvectorCentrality(this.project.getId())) {
    ...
    public Class<? extends DataAnalysisBase> describe() {
        return EigenvectorCentralityAnalysis.class;
    }
}
